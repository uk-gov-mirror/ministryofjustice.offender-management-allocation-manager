...
feature "view POM's caseload" do
  ...
  let(:offender_ids_by_release_date) {
    offender_map.excluding('G8339GD').map { |k, v| [k, v] }.sort_by { |_k, v| v }.map { |k, _v| k }
  }
  let(:offenders) {
    offender_map.merge(nomis_offender_id => 1_153_753).
      ...
      build(:nomis_offender,
            ...
            sentence: attributes_for(:sentence_detail,
                                     ...
                                     tariffDate: (nomis_id == 'G8339GD') ? nil : Time.zone.today + booking_id.days))
                                     #tariffDate: Time.zone.today + booking_id.days))
    }
  }
